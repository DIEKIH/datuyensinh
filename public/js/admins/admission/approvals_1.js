$(function () {
    ...
    $('#runNurtureCommand').on('click', function () {
        ...
        $button
            .prop('disabled', true)
            .html('<i class="fas fa-spinner fa-spin me-1"></i>Đang chạy...');

        $.post('/admin/admission-cms/nurture')
            .done(function (response) {
                toastr.success(
                    response.message || 'Đã chạy kịch bản nurturing.'
                );
                loadApprovals(1, { silent: true });
            })
            .fail(function (xhr) {
                toastr.error(
                    Cms.errorMessage(xhr, 'Không thể chạy nurturing.')
                );
            })
            .always(function () {
                $button.prop('disabled', false).html(oldHtml);
