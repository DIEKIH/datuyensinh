...
$('#baivietTable').on('click', '.edit-btn', function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            // ✅ Fill dữ liệu sự kiện nếu có
            if (bv.thoigian_bd || bv.thoigian_kt || bv.diadiem) {
                if (bv.thoigian_bd) {
                    // Chuyển "2025-09-23 14:35:25" → "2025-09-23T14:35" cho datetime-local
...
$.ajax({
    type: "get",
    url: "/admin/tacgia/list", // route Laravel trả về getTacgia()
    success: function (res) {
        if (res.success) {
            loadTacgia(res.data);
        }
...
// Hiện/ẩn block khi đổi danh mục
$('#danhmuc').on('change', function () {
    if (isSuKien()) {
        ...
        // Xóa dữ liệu khi ẩn để không gửi lên server
        $('#thoigian_bd, #thoigian_kt, #diadiem').val('');
        $('#sukienStatus').val('').css('color', '');
    }
});
    ...
    $('#baivietModal').one('hidden.bs.modal', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // Loại bài đang sửa khỏi danh sách
                ...
                // Tick lại các bài đã chọn trước
                setTimeout(function () {
